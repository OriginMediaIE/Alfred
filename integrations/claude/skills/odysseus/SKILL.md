---
name: odysseus
description: Use when the user asks Claude Code to read or write OM Automate data (todos, email, calendar, memory, documents) or to launch/monitor/stop a Cookbook model-serve task through the scoped Claude Agent API. Requires ODYSSEUS_URL and ODYSSEUS_API_TOKEN.
---

# OM Automate

Use this skill when a user asks to interact with OM Automate from Claude Code.

## Configuration

Expect these environment variables:

The `odysseus` skill ID, helper filename, and `ODYSSEUS_*` variable names are
published compatibility identifiers. Keep them unchanged while presenting the
product to users as OM Automate.

- `ODYSSEUS_URL`: Base URL for the user's OM Automate instance, for example `http://127.0.0.1:7000`.
- `ODYSSEUS_API_TOKEN`: Scoped API token created in OM Automate Settings > Integrations > Add Integration > Claude Agent.

If either value is missing, do not guess credentials. Tell the user to create a Claude Agent token in OM Automate Settings and expose both values to the terminal session.
...
- **Calendar event ("meeting at 3pm", "dentist Tuesday 10am")** → calendar event. Use for scheduled time blocks, meetings, appointments, recurring schedules. These show up on the calendar grid; reminders for them are configured separately in OM Automate settings.
...
- All OM Automate data access MUST go through the scoped HTTP API under `/api/codex/*` (the canonical scope-gated agent API, shared by all agent integrations).
...
- Do not use SSH, Docker, direct Python imports, SQLite queries, MCP internals, browser cookies, or local files to read/write OM Automate user data.
...
- Prefer `POST /api/codex/emails/draft-document` for agent-written email replies. It creates an editable OM Automate Document with `language: "email"` and does not touch IMAP/send.
...
If you are about to reach the OM Automate host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass OM Automate Settings and token scopes. Ask the user to enable the relevant Claude Agent tool toggle instead.
