...
def _create_email_draft_document(
    *,
    ...
    """Create an OM Automate email compose document for user review. Does not send."""
...
def _draft_reply_to_email(uid, body, folder="INBOX", reply_all=False, account=None, title=None):
    """Create a threaded OM Automate reply draft document. Does not send."""
    conn = _imap_connect(account)
    conn.select(_q(folder), readonly=True)
    status, msg_data = conn.uid("FETCH", _b(uid), "(BODY.PEEK[])")
    conn.logout()
...
async def _ai_draft_reply_to_email(uid, folder="INBOX", reply_all=False, account=None, title=None):
    """Generate a reply with OM Automate's AI-reply prompt/style, then create a compose doc."""
...
async def list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            ...
            description=(
                "List the email accounts configured in OM Automate. Returns each account's "
        ...
        Tool(
            ...
            description=(
                ...
                "draft_email so the user can review and send from OM Automate. "
        ...
        Tool(
            name="draft_email",
            description=(
                "Create a new OM Automate email compose draft document. This DOES NOT send. "
                ...
                "can edit or press Send in OM Automate. "
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "cc": {"type": "string", "description": "CC address(es), comma-separated (optional)"},
                    "bcc": {"type": "string", "description": "BCC address(es), comma-separated (optional)"},
                    "title": {"type": "string", "description": "Optional OM Automate document title"},
                    **ACCOUNT_PROP,
                },
                "required": ["to", "subject", "body"],
            },
        ...
        Tool(
            ...
            description=(
                ...
                "draft_email_reply so the user can review and send from OM Automate. "
        ...
        Tool(
            ...
            description=(
                "Create an OM Automate email reply draft document for an existing email UID. "
        ...
        Tool(
            ...
            description=(
                "Generate an AI reply using OM Automate's existing AI Reply behavior, "
    ...
    try:
        ...
        if name == "list_emails":
            ...
            if result.get("pending"):
                return [TextContent(
                    ...
                    text=(
                        ...
                        "Nothing has been sent yet. Review and approve it in OM Automate before delivery."
                    ),
                )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created OM Automate email draft `{result['title']}` "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in OM Automate to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created OM Automate reply draft `{result['title']}` for UID {uid} "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in OM Automate to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Generated AI reply and created OM Automate compose draft "
