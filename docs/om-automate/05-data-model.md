...
| work_projects | title, goal, desired outcome, status/area, notes, risks, decisions, tags, optional budget, start/target dates, progress summary, creator/approval/action/revision | Parent of work_milestones; referenced by work_tasks and work_commitments | non-null exact owner key; empty string is the deliberate auth-disabled single-user tenant | Canonical Phase Ten project aggregate. Risks/decisions/tags are bounded canonical JSON; budget uses integer minor units plus ISO currency. |
| work_milestones | title/description, status, target/completion dates, sort order and revision | project_id cascade delete; work_tasks milestone link becomes null | duplicates the exact project owner for fail-closed checks | Canonical project milestone. Service rejects cross-owner and project/milestone mismatches. |
| work_tasks | title/description/status/priority, start/due/completion, estimates/actuals, project/milestone/parent, area/tags/contexts/assignees/energy/effort, recurrence, source, completion evidence, creator/approval/action, legacy projection link and revision | Project/milestone set-null, parent cascade, dependency/reminder/reference children | non-null exact owner key; every service query matches it | Canonical personal-work record. A unique read-only `legacy_scheduled_task_id` projection preserves automation visibility without changing `scheduled_tasks`. |
| work_task_dependencies | directed task → prerequisite edge | both task IDs cascade delete; unique edge | exact owner copied and verified against both tasks | Canonical dependency graph. The service rejects self, cross-owner and transitive cyclic edges before commit. |
| work_reminders | entity type/id, due time, message/channel/status, recurrence and fired time | Polymorphic task/commitment reference; service-owned cleanup | exact owner key | Canonical local reminder metadata. It does not silently create a scheduler job or external notification effect. |
| work_commitments | promise/title/description, status and review state, due/fulfilled time, counterparty, project/task, structured source, confidence, creator/action and completion notes | Optional project/task; typed reference/reminder children | non-null exact owner key | Canonical source-linked commitment. Agent-extracted records default to suggested; source type is required and review transitions are validated. |
| work_references | entity type/id, reference type, external ID, label, URL and metadata | Polymorphic reference to task/project/commitment | exact owner key | Canonical link to email, meeting, document, calendar event, contact, note, URL or other source; provider records remain authoritative. |
| work_plans | focus/breakdown/reschedule type, goal/date, status, proposal/work-block/assumption JSON, creator/action, apply time and revision | Applying a breakdown creates work_tasks; reschedule updates task due dates | non-null exact owner key | Canonical correctable planning draft. Revision compare-and-set prevents stale apply; work blocks have no implicit calendar side effect. |
| work_mutation_receipts | entity/operation, actor kind/id, action/correlation IDs, before/after hashes, bounded details and time | Joins agent-originated writes to agent_actions by action_id | non-null exact owner key | Append-only domain receipt. Agent mutations require the owner-matching action ledger row for the exact manage/delete tool to be in executing state. |
