...
| Personal work | `/api/work/tasks*`, `/projects*`, `/commitments*`, `/plans*`, `/planning/*`, `/reminders/due`, `/audit` | Additive Phase Ten API for owner-scoped personal work. Mutations use optimistic revisions and domain receipts; agent handlers additionally require a matching claimed generic action-ledger record. The router must be included by the application integration before deployment. |
