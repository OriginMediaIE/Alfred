...
PLAN_MODE_READONLY_TOOLS = PLAN_MODE_ALLOWED_TOOL_NAMES
...
def plan_mode_disabled_tools() -> Set[str]:
    ...
    Plan mode allows only PLAN_MODE_READONLY_TOOLS. The execution gate is a
    denylist, so return the exact inverse across every canonical static tool.
    This projection has no schema, facade, handler, or app import path and cannot
    fail open because of import order. Dynamic MCP tools are filtered separately.
    """

    return set(BUILTIN_TOOL_NAMES - PLAN_MODE_READONLY_TOOLS)
