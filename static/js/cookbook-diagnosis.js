...
function _gpuCleanupCommand() {
  return `set -u
echo "[om-automate] Clearing GPU compute processes..."
if command -v nvidia-smi >/dev/null 2>&1; then
  pids="$(nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits 2>/dev/null | tr -d " " | grep -E "^[0-9]+$" | sort -u)"
  if [ -z "$pids" ]; then
    echo "[om-automate] No NVIDIA compute processes found."
    exit 0
  fi
  echo "[om-automate] GPU PIDs: $pids"
  ps -fp $pids 2>/dev/null || true
  echo "[om-automate] Sending TERM..."
  kill -TERM $pids || true
  sleep 3
  alive=""
  for pid in $pids; do
  ...
  if [ -n "$alive" ]; then
    echo "[om-automate] Force killing remaining GPU PIDs:$alive"
  ...
  if [ -n "$remaining" ]; then
    echo "[om-automate] GPU processes still remain:"
    echo "$remaining"
    exit 2
  fi
  echo "[om-automate] GPU cleanup complete. No NVIDIA compute processes remain."
else
  echo "[om-automate] nvidia-smi not found; falling back to common model-server process cleanup."
  ...
  echo "[om-automate] Fallback cleanup complete."
fi`;
}
...
export const ERROR_PATTERNS = [
  ...
  },
  {
    ...
    fixes: [
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('vllm') },
      {
        ...
        action: () => _copyText('Upgrade the vLLM environment that provides the selected vllm CLI, or use a compatible checkpoint. Do not assume OM Automate owns PATH/system/source/Docker installs.'),
      },
    ],
  },
  {
  ...
  },
  {
    ...
    suggestion: 'Suggested action: relaunch with OM Automate’s venv CUDA library path fix. If the venv does not contain the matching NVIDIA runtime libs, run Repair sglang-kernel.',
    fixes: [
  ...
  },
  {
    ...
    suggestion: 'Suggested action: relaunch from the cached local snapshot path. OM Automate rewrites MLX repo-id launches to the newest local Hugging Face snapshot when it exists on the selected Mac.',
    fixes: [
      { label: 'Edit / relaunch serve', action: (panel) => _openServeEditFromDiagnosis(panel) },
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('mlx_lm') },
      { label: 'Copy error', action: (panel) => {
...
function _diagnosisCopyBundle(task, diagnosis, sourceText, suggestionText) {
  const lines = ['## OM Automate model runtime troubleshooting'];
  if (task) {
    lines.push(
      '',
      '### Task',
